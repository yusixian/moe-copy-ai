...
export const Segmented = <T extends string | number = string | number>({
  ...
  return (
    <div
      className={cn(
        "flex w-fit cursor-pointer select-none rounded-lg border border-line-1 bg-content p-1 font-medium text-xs",
        className
      )}>
      {options.map((option) => {
          ...
          <motion.div
            ...
            {selected && (
              <motion.div
                ...
                className={cn(
                  "absolute inset-0 -z-10 rounded-md border border-line-1 bg-content-alt-solid",
                  indicateClass
                )}
                transition={
                  shouldReduceMotion
