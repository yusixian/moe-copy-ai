      ...
      <div className="flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="输入正则表达式..."
            className={cn(
              'w-full rounded-md border py-1.5 pl-8 pr-8 text-sm transition-colors focus:outline-none focus:ring-1',
              !isValid
        ...
        <div className="relative" ref={presetRef}>
          ...
          >
            ...
            <Icon
              ...
              className={cn('h-4 w-4 transition-transform', isPresetOpen && 'rotate-180')}
            />
          </button>
            ...
            {isValid ? (
              <>
                <Icon
                  ...
                  className={cn('h-3.5 w-3.5', filteredCount > 0 ? 'text-sky-500' : 'text-gray-400')}
                />
                <span className="text-gray-500">
                  {mode === 'exclude' ? '已排除' : '已保留'}{' '}
                  <span className={cn('font-medium', filteredCount > 0 ? 'text-sky-600' : 'text-gray-600')}>
              ...
              <span className="text-red-500">
                <Icon icon="mdi:alert-circle" className="mr-1 inline h-3.5 w-3.5" />
                {error || '正则无效'}
              </span>
            )}
          </div>
