...
const ScrapeResultsPanel = memo(function ScrapeResultsPanel({
  ...
  const [fullContentItems, setFullContentItems] = useState<Set<number>>(new Set())
      ...
      <div className="flex items-center justify-between rounded-lg border border-gray-200 bg-gray-50 px-3 py-2">
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={selectAll}
            className="flex items-center gap-1 rounded px-2 py-1 text-xs text-sky-600 hover:bg-sky-100"
          >
          ...
          >
            <Icon icon="mdi:checkbox-multiple-blank-outline" className="h-3.5 w-3.5" />
            取消
          </button>
        </div>
        <span className="text-xs text-gray-500">
          已选 <b className="text-sky-600">{selectedStats.count}</b> / {stats.successCount}
        </span>
      </div>
          ...
          >
            ...
            >
              ...
              {result.success ? (
                ...
                >
                  <Icon
                    icon={isSelected(result.url) ? 'mdi:checkbox-marked' : 'mdi:checkbox-blank-outline'}
                    className={cn(
                      'h-4 w-4',
                ...
                <Icon icon="mdi:checkbox-blank-off-outline" className="h-4 w-4 flex-shrink-0 text-gray-300" />
              )}
              {result.success ? (
              ...
              <span className={cn('flex-1 truncate text-sm', result.success ? 'text-gray-700' : 'text-gray-400')}>
                {result.title}
              </span>
              <Icon
            ...
            {expandedIndex === index && (
              <div className="border-t border-gray-100 bg-white px-3 py-2">
                ...
                <div className="mb-2 flex items-center justify-between gap-2">
                  <span className="flex-1 truncate text-xs text-gray-500">{result.url}</span>
                  <button
                    onClick={(e) => {
                      e.stopPropagation()
                ...
                {result.success ? (
                  <div className="rounded bg-gray-50 p-2">
                    <div
                      className={cn(
                        'overflow-y-auto whitespace-pre-wrap break-words text-xs text-gray-600',
                    ...
                    >
                      ...
                      {result.content.length > 500 && !fullContentItems.has(index) && '...'}
                    </div>
                    {result.content.length > 500 && (
                      <button
                        ...
                        className="mt-2 flex w-full items-center justify-center gap-1 rounded border border-gray-200 bg-white py-1 text-xs text-gray-500 transition-colors hover:bg-gray-50"
                      >
                        <Icon
