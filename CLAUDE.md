...
- `useI18n()` - Internationalization and translation

## Internationalization (i18n)

**Core Rule**: NEVER hardcode user-visible text. Always use `useI18n()` + translation keys.

### Key Organization

- Namespaces: `common.*`, `popup.*`, `batch.*`, `error.*`, `toast.*`
- Naming: Dot notation, descriptive (`error.apiKeyNotSet` not `error.e1`)

### Checklist

**New feature**: Add keys to both `zh_CN.json` + `en_US.json`, use `t()` from start
**Before commit**: `wc -l locales/*.json` (same count?), `pnpm build` (no warnings?)

### Special Cases

- **Hooks**: Include `t` in `useCallback` deps
- **Background**: Keep errors in English (no React context)
- **Constants**: Use `labelKey`/`descKey`
